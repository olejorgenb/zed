...
impl MultiBuffer {
    ...
    /// Rewrites the excerpts of every path touched by `anchors`, applying `adjust` to the
    /// displayed range of each excerpt that contains one of them.
    ///
    /// Expanding, contracting and snapping to a syntax node all differ only in that closure;
    /// keeping the traversal in one place means a fix to how excerpts are located or written
    /// back reaches all of them.
    fn adjust_excerpt_ranges(
        &mut self,
        anchors: impl IntoIterator<Item = Anchor>,
        cx: &mut Context<Self>,
        adjust: impl Fn(&mut ExcerptRange<Point>, &BufferSnapshot),
    ) {
        let snapshot = self.snapshot(cx);
        ...
        for (path_index, excerpt_anchors) in &buffers {
                ...
                && &excerpt.path_key == path
            {
                ...
                if needs_adjust {
                    adjust(&mut range, buffer_snapshot);
                }
    ...
    pub fn expand_excerpts(
        ...
        self.adjust_excerpt_ranges(anchors, cx, |range, buffer_snapshot| {
            if direction.should_expand_up() {
                range.context.start.row = range.context.start.row.saturating_sub(line_count);
                range.context.start.column = 0;
            }
            if direction.should_expand_down() {
                ...
                range.context.end.column = buffer_snapshot.line_len(range.context.end.row);
            }
        })
    }
    ...
    pub fn expand_excerpts_to_syntax_node(
        ...
        self.adjust_excerpt_ranges(anchors, cx, |range, buffer_snapshot| {
            if let Some(expanded) =
                enclosing_syntax_node_range(buffer_snapshot, range.context.clone(), direction)
            {
